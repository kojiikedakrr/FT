...
####パラメータ推計####
set.seed(19921002)
lower_bounds = c(rep(-3,LATENT_DIM*(LATENT_DIM+1)/2),rep(-3,LATENT_DIM),rep(-3,LATENT_DIM*(LATENT_DIM+1)/2),rep(-10,LATENT_DIM),c(-15,-15,-20,-15,-20,-15,-20),-1)
upper_bounds = c(rep(5,LATENT_DIM*(LATENT_DIM+1)/2),rep(3,LATENT_DIM),rep(3,LATENT_DIM*(LATENT_DIM+1)/2),rep(-5,LATENT_DIM),c(-3,-3,-19.99,-3,-19.99,-3,-19.99),1)
optim_params = DEoptim(build_dlm_forDE, lower = lower_bounds, upper = upper_bounds,control=list(NP = 20*50,itermax = 500, F = 0.8, CR = 0.9))
...
g_spot2 = ggplot(plot_dat) +
  ...
  scale_y_continuous(breaks=seq(-2,10,2),limits=c(-1,8),labels = str_c(seq(-2,10,2),'%'))
plot(g_spot2)
